...
namespace mm
{
    ...
    bool Retile::retileOneFileBySize(std::string &fileName)
    {
        ...
        std::filesystem::create_directories(this->m_outFolder.c_str());
        ...
        for (int i = 0; i < this->m_numRows; ++i)
        {
            for (int j = 0; j < this->m_numCols; ++j)
            {
                ...
                std::string outPath = this->m_outFolder + "/" + pathObj.stem().c_str();
    ...
    bool Retile::retileOneFileByCount(std::string &fileName)
    {
        ...
        double deltaX = (reader->header.max_x - reader->header.min_x) / this->m_numCols + 0.001;
        double deltaY = (reader->header.max_y - reader->header.min_y) / this->m_numRows + 0.001;
        ...
        std::vector<int> numPts(this->m_numCols * this->m_numRows);
        std::vector<LASwriter *> writers(this->m_numCols * this->m_numRows);
        std::cout << "Total number of tiles: " << this->m_numCols * this->m_numRows << std::endl;
        ...
        std::vector<double> xxDelta;
        std::vector<double> xxCount;
        std::vector<double> yy;
        std::vector<double> yyDelta;
        std::vector<double> yyCount;
        while (reader->read_point())
        {
            ...
            if (row >= 40)
            {
                int kkk = 0;
                countRow++;
                yy.push_back(reader->point.get_y());
                yyDelta.push_back(reader->point.get_y() - reader->header.min_y);
                yyCount.push_back((reader->point.get_y() - reader->header.min_y) / deltaY);
                continue;
            }
            if (col >= 40)
            {
                countCol++;
                int kk = 0;
                xx.push_back(reader->point.get_x());
                xxDelta.push_back(reader->point.get_x() - reader->header.min_x);
                yyCount.push_back((reader->point.get_x() - reader->header.min_x) / deltaX);
                continue;
            }
            numPts[index]++;
